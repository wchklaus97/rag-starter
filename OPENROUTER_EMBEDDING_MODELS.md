...
> **Note:** the same file is copied to [`rag_model_site/OPENROUTER_EMBEDDING_MODELS.md`](./rag_model_site/OPENROUTER_EMBEDDING_MODELS.md) for the static GitHub Pages field guide. Run `uv run python scripts/collect_openrouter_models.py` to refresh that copy when you use an OpenRouter API key.
